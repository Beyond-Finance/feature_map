...
import { getFeatureSizeLabel, getFilledPills, renderTeams } from '../utils/feature-helpers';
...
const highlightFeatures = ({ features, metric, sortDirection = 'ASC' }) => {
  return Object.entries(features)
    .sort(([_a, featureA], [_b, featureB]) => {
      ...
      if (metricA === metricB) {
        return (
          featureB.additional_metrics.feature_size.score -
          featureA.additional_metrics.feature_size.score
        );
      }
            ...
            <table className="min-w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <TableHeader
                    title={viewType === 'Missing' ? `${testType} Test Count` : 'Pending Test Count'}
                  />
                  <th
                    scope="col"
                    className="px-4 py-3 text-left text-xs font-medium text-gray-800 uppercase tracking-wider"
                  ...
                  return (
                    <tr key={featureName}>
                      ...
                      <td className="px-4 py-4 text-sm">
                        <div className="flex items-center gap-x-2">{score}</div>
                      </td>
                      <td className="px-4 py-4 text-sm text-gray-500">
