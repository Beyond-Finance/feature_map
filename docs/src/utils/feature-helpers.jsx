...
export const getFeatureSizeColor = (sizeScore) => {
  if (sizeScore === undefined || sizeScore === null)
    return { hex: '#f3f4f6', class: 'bg-gray-100' };
...
export const getTestCoverageColor = (coverageScore) => {
  if (coverageScore === undefined || coverageScore === null)
    return { hex: 'transparent', class: 'bg-transparent' };
  if (coverageScore >= testCoverageThresholds.good)
    return { hex: '#22c55e', class: 'bg-green-500' };
  if (coverageScore >= testCoverageThresholds.fair)
    return { hex: '#eab308', class: 'bg-yellow-500' };
...
export const renderTeams = (teams) => {
  if (!teams || teams.length === 0) {
    return <span className="text-gray-400 italic text-sm">No team assigned</span>;
  }
