RSpec.describe FeatureMap::CodeFeatures do
  ...
  before do
    write_file('config/features/my_feature.yml', feature_yml)
  ...
  describe '.all' do
    it 'correctly parses the feature files' do
      ...
      expect(feature.config_yml).to eq 'config/features/my_feature.yml'
    end
    ...
    context 'feature YML has syntax errors' do
      ...
      it 'spits out a helpful error message' do
        expect { FeatureMap::CodeFeatures.all }.to raise_error do |e|
          ...
          expect(e.message).to eq('The YML in config/features/my_feature.yml has a syntax error!')
        end
      end
    end
  end
  ...
  describe 'validation_errors' do
    ...
    context 'there are multiple definitions for the same feature' do
      before do
        write_file('config/features/my_other_features.yml', feature_yml)
      end
